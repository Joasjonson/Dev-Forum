...
def show_post(post_id):
    ...
    if current_user == post.author:
        ...
        if request.method == 'GET':
            ...
            db.session.commit()
            flash('Your post has been updated successfully.', 'alert-success')
            return redirect(url_for('forum'))    
    else:
        form = None
    return render_template('posts.html', post=post, form=form, current_user=current_user)
...
def delete_post(post_id):
    ...
    if current_user == post.author:
        ...
        db.session.commit()
        flash('Your post has been deleted successfully.', 'alert-danger')
        return redirect(url_for('forum'))
    else:
        abort(403)     
